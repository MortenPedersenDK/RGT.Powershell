...
namespace RGT.Services.Core.API
{
    public class RGTAPI
    {
        ...
        public async Task<string> CreateEventWithBranding(Context ctx, EventCreationInformation creationInformation)
        {
            ...
            var creationResponse = JsonSerializer.Deserialize<EventWithBrandingCreationResponse>(str);
        ...
        public async Task<string?> AttachMagicRoadAsync(Context ctx, string id)
        {
            ...
            var roadMagicAttach = JsonSerializer.Deserialize<RoadMagicAttach>(json);
        ...
        {
            var client = GetHttpClient(ctx);
            var response = await client.GetFromJsonAsync<RoadsMagicResponse>($"/v1/roads/magic?limit={limit}&order=name&direction=asc");
        ...
        public async Task<IList<RoadsMagicResponse.Road>?> SearchMagicRoadsByName(Context ctx, string namePartial, int limit = 5, int offset = 0)
        {
            ...
            var response = await client.GetFromJsonAsync<RoadsMagicResponse>($"/v1/roads/magic?limit={limit}&offset={offset}&name={nameUrlEncoded}");
            if (response != null && response.status == 200 && response.data != null)
            {
                return response.data.ToList();
            }
        ...
        public async Task<Context?> Login(string username, string password)
        {
            ...
            var result = await client.PostAsJsonAsync("/v1/auth/login", new LoginRequest() { email = username, password = password });
